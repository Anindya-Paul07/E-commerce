...
export async function list(req, res, next) {
  try {
    ...
    const [items, total] = await Promise.all([
      Warehouse.find(filter).sort({ isDefault: -1, code: 1 }).skip((page - 1) * limit).limit(limit),
...
export async function create(req, res, next) {
  try {
    ...
    if (doc.isDefault) await Warehouse.updateMany({ _id: { $ne: doc._id } }, { $set: { isDefault: false } })
